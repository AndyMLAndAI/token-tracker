...
<head>
  <meta charset="UTF-8">
  <title>${escapeHtml(reportTitle)} — ${escapeHtml(startStr)} to ${escapeHtml(endStr)}</title>
  <style>
    :root {
      --bg: #ffffff;
      --text: #111111;
  ...
  <div class="container">
    <header>
      <div>
        ...
        <div class="meta">Period: <strong>${escapeHtml(startStr)}</strong> — <strong>${escapeHtml(endStr)}</strong></div>
        ${brandingMeta ? `<div class="branding-sub">Prepared by ${escapeHtml(brandingMeta)}</div>` : ''}
      </div>
      <div class="no-print">
    ...
    <table>
      ...
      <tbody>
        ${report.toolBreakdown
          .map(
            (t) => `<tr>
          <td><strong>${escapeHtml(t.toolSource)}</strong></td>
    ...
    <footer>
      <span>Generated by Token Tracker ${brandingMeta ? `(${escapeHtml(brandingMeta)})` : '(Exact Local Telemetry)'}</span>
